...
func main() {
	...
	mv := NewMajorityVotingDynamic(n, func(i int) int { return a[i] })
	for i := 0; i < q; i++ {
		var op int
		fmt.Fscan(in, &op)
		if op == 0 {
			...
			var l, r int
			fmt.Fscan(in, &l, &r)
			majority, freq := mv.Query(l, r)
			if freq*2 > r-l {
				fmt.Fprintln(out, majority)
			} else {
				fmt.Fprintln(out, -1)
			}
		}
...
type MajorityVotingDynamic struct {
	seg       *SegmentTree
	rangeFreq *RangeFreqDynamic
}

func NewMajorityVotingDynamic(n int, f func(int) int) *MajorityVotingDynamic {
	seg := NewSegmentTree(n, func(i int) E { return of(f(i)) })
	rangeFreq := NewRangeFreqDynamic(n, f)
	return &MajorityVotingDynamic{seg: seg, rangeFreq: rangeFreq}
}

func (mv *MajorityVotingDynamic) Set(index int, value int) {
	ok := mv.rangeFreq.Set(index, value)
	if ok {
		mv.seg.Set(index, of(value))
	}
}

func (mv *MajorityVotingDynamic) Query(start, end int) (majority int, freq int) {
	majority = mv.seg.Query(start, end).value
	freq = mv.rangeFreq.Query(start, end, majority)
	return
}
...
type RangeFreqDynamic struct {
	data           []int
	valueToIndexes map[int]*SortedList
}

func NewRangeFreqDynamic(n int, f func(int) int) *RangeFreqDynamic {
	data := make([]int, n)
	for i := 0; i < n; i++ {
		data[i] = f(i)
	}
	...
	for v, indexes := range mp {
		valueToIndexes[v] = NewSortedList(func(a, b int) bool { return a < b }, indexes...)
	}
	return &RangeFreqDynamic{data: data, valueToIndexes: valueToIndexes}
}

func (r *RangeFreqDynamic) Query(start, end int, value int) int {
	if start >= end {
		return 0
	}
	pos, ok := r.valueToIndexes[value]
	if !ok {
		return 0
	}
	return pos.BisectLeft(end) - pos.BisectLeft(start)
}

func (r *RangeFreqDynamic) Set(index int, value int) bool {
	pre := r.data[index]
	if pre == value {
		return false
	}
	pos, ok := r.valueToIndexes[pre]
	if ok {
		pos.Discard(index)
	}
	r.data[index] = value
	pos, ok = r.valueToIndexes[value]
	if !ok {
		pos = NewSortedList(func(a, b int) bool { return a < b })
		r.valueToIndexes[value] = pos
	}
	pos.Add(index)
	return true
}

func (r *RangeFreqDynamic) Get(index int) int {
	return r.data[index]
}
