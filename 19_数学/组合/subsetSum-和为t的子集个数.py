...
def linear_recurrence(arr, coeff, k):
    ...
    p = (f * q).resize(d)
    while k:
        r = FormalPowerSeries([q[i] * (1 - (i & 1) * 2) for i in range(len(q))])
        r = r.resize(d + 1)
        p *= r
        q *= r
        p = p[(k & 1) :: 2]
...
def stirling_second(n):
    ...
    p = FormalPowerSeries([pow(i, N, MOD) * inv[i] % MOD for i in range(n + 1)])
    q = FormalPowerSeries([inv[i] * (1 - (i & 1) * 2) for i in range(n + 1)])
    res = (p * q)[: n + 1]
    return res
...
def bernoulli(n):
    fct, inv = build_factorial(n + 1)
    p = FormalPowerSeries([0] * (n + 1))
    p[0] = 1
    expx = FormalPowerSeries([inv[i + 1] for i in range(n + 1)])
    q = p / expx
    res = FormalPowerSeries([q[i] * fct[i] % MOD for i in range(n + 1)])
    return res
...
def eulerian(n):
    fct, inv = build_factorial(n + 1)
    p = FormalPowerSeries([pow(i + 1, n, MOD) for i in range(n + 1)])
    q = FormalPowerSeries([inv[i] * inv[n + 1 - i] * (1 - (i & 1) * 2) % MOD for i in range(n + 1)])
    res = (p * q * fct[n + 1])[:n]
    return res
...
def subset_sum(a, k):
    ...
    for i in range(2, k + 1):
        inv[i] = -inv[MOD % i] * (MOD // i) % MOD
    b = FormalPowerSeries([0] * (k + 1))
    for i in range(1, k + 1):
        for j in range(i, k + 1, i):
            b[j] += inv[j // i] * cnt[i] * ((j // i & 1) * 2 - 1)
            b[j] %= MOD
    res = b.exp()
    return res
...
print(subset_sum(nums, t)[1:])  # 和为1,2,...,t的方案数
