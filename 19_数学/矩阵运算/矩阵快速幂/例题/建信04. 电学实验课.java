...
class Solution {
  public int electricityExperiment(int row, int col, int[][] position) {
    ...
    Arrays.sort(position, (a, b) -> a[1] - b[1]);

    long[][] T = new long[row][row];
    for (int i = 0; i < row; i++) {
      T[i][i] = 1;
      if (i > 0) {
        T[i][i - 1] = 1;
    ...
    MatPow mp = new MatPow(T, MOD, 4);
    long res = 1;
    for (int i = 0; i < n - 1; i++) {
      int row1 = position[i][0];
      int row2 = position[i + 1][0];
      int col1 = position[i][1];
      int col2 = position[i + 1][1];
      int len = col2 - col1;
      long[][] mat = mp.pow(len);
      res = (res * mat[row1][row2]) % MOD;
    }
    return (int) res;
  }
}
