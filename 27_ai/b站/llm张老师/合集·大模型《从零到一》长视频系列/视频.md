...
### 大模型推理 vs. 训练：本质差异与核心逻辑

1. **本质区别：权重状态**
   - **训练 (Training)**：动态过程。通过反向传播计算梯度，不断更新参数权重以最小化损失函数 (Loss)。
   - **推理 (Inference)**：静态过程。参数完全冻结，仅利用现有权重进行前向计算 (Forward Pass)。

2. **核心机制：自回归生成 (Autoregression)**
   - 推理是**单步滚动**的：每次只预测下一个 Token，并将其拼接到输入序列中，作为下次推理的上下文。
   - **滑动窗口**：受限于算力，输入序列长度（Context Length）存在上限。当超出长度时，需舍弃最早的 Token。

3. **算力瓶颈：$O(n^2)$ 复杂度**
   - 注意力机制的特征矩阵计算量随上下文长度 $n$ 呈平方级增长。扩大窗口（如从 1k 到 1M）的主要障碍是硬件显存与计算密度，而非算法。

4. **架构演进**
   - 现代大模型（如 GPT 系列）多采用 **Only-Decoder** 架构，相比传统的 Encoder-Decoder，它在处理海量预测任务时更易扩展、计算效率更高。

#### 简易推理逻辑示例 (Python)

```python
def generate_text(model, input_tokens, max_len=50):
    # 推理循环：一个字一个字生成
    for _ in range(max_len):
        # 保持权重不变的条件下进行前向计算
        # context_window 保持在模型定义的 context_length 内
        logits = model.forward(input_tokens[-128:])

        # 选取概率最高的新token
        next_token = torch.argmax(logits[-1, :])

        # 将新生成的token拼接回输入，进行下一次预测
