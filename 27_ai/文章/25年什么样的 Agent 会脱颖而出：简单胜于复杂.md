...
- 这个不太认同，多 agent 的延迟/成本目前没有啥好方案，25 年一个比较明显的趋势在于很多 subagent 直接被工具替代，另一个强信号在于 anthropic 提出的 skills，后续大概率多 Agent 架构都会向单 agent+skill 的架构转变
- 毫不否认 ai 越来越强的情况下，单体式的必然性和趋势性，但目前来看离稳定、工业级的实现还很远，所以你可以把这个当做 过渡式的、奇技淫巧的策略。
  且 skills 类的方法早在 24 年就有很多实现，没有被广泛采纳很大程度上是因为没有如同 claude 一样优秀的大模型，这类技艺过于吃模型性能了，而工业部署很多都是 7-9b 的小模型。
