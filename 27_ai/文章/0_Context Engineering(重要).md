...
- Agent 的 context 大致分为两类：相对静态的部分（System Prompt、工具定义、领域知识）和动态增长的部分（对话历史、工具调用记录、检索到的知识）。静态部分在会话开始时就确定了，占据固定的 token 预算；动态部分随着任务执行不断膨胀。Context Engineering 需要同时管理两者
  - 静态
    - CLI 化
    - **渐进式披露**
    - Tool Search Tool：不要把所有工具定义一次性塞给模型，而是先给模型一个检索工具的工具
  - 动态(即使模型变得更强也不会自动变好的东西：信噪比优化、经验复用、目标驱动的裁剪)
    - 工具Observation 为主
      在ReAct循环中，真正吃掉context 的大头是 Observation，也就是每次工具调用返回的结果。Agent 自己产生的 Thought 和 Action 通常只有几行，但一次 cat 命令可能返回几百行代码，一次测试执行可能吐出上千行日志。随着循环轮次增加，这些 Observation 层层叠加，迅速占满 context window
    - 减少运行时噪音
      - 真正需要 Agent 仔细分析的，是 3 个 fail 的堆栈跟踪和错误信息。而这 3 条关键信息被淹没在 97 条成功日志的海洋里，简单直接方案是：针对我们常用的测试方案和测试库，去积累一些特殊的为 AI 使用的测试脚本。当然，这些脚本也可以是 AI 帮助生成的，它们仅包含 AI 关注的那 3 个错误信息
    - 工具执行独立化：工具为不一定要把原始输出直接塞回主对话上下文，模型拿到的只是一段高度压缩的摘要，或者一个可以继续检索的句柄
      参考：openViking
      - 工具负责执行动作
      - 存储层负责保留原始结果
      - 检索层负责按需召回片段
      - 模型只消费当前决策真正需要的高信号部分
    - 经验工程(怎么能为 AI 提供更多 AI-native 的工具和工作流)
      - AGENTS.md 保持精简
      - OpenAI Codex 团队会在仓库中建立一个结构化的 docs/ 目录：设计文档、执行计划、技术债跟踪、外部参考都按类别分目录存放，Agent 根据当前任务按需查阅
      - 在任何困难或运行长时间的任务完成之后输入

        ```md
        将本次任务经验写入文档。

        <target_file>
        docs/experiences/xxx
        </target_file>
        ...
        请生成完整的 Markdown 文档内容。
        ```

- Coding Agent 其实是一个读多写少的系统
  read 类操作消耗了整整 76.1% 的 token，execute 占 12.1%，edit 仅占 11.8%
- 更少噪音 = 更精准的决策
  Anthropic 的 Context Rot 理论一致，更短但更精准的 context，胜过更长但充满噪音的 context
- Context Engineering 说到底就是别让模型把算力浪费在读垃圾上。静态层面控制工具数量，动态层面压缩噪音、复用经验、按目标裁剪。
