...
- 将操作变换（OT）和冲突自由复制数据类型（CRDT）严格区分并没有真正的理由。支持去中心化编辑的 OT 算法更像是 CRDT 算法。但把愚钝的经典 OT 与具备去中心化能力的方法区分开来仍然是有用的。
- ProseMirror 使用的是类似于 OT 的方法，但`没有收敛的变换函数`。它可以将更改相互转换，但不能以保证收敛的方式进行。
- 当客户端收到有冲突的远程更改时，它首先撤销所有本地待定更改，应用远程更改，然后再应用本地更改的变换形式。这意味着每个人最终都会`以相同的顺序应用更改，因此收敛是显而易见的。`
- 在 CodeMirror 中，`文档模型要简单得多`，所以尽管我用 ProseMirror 的方法走得挺远，但它显得不必要地别扭。对文档进行一系列独立的更改，`每一项都应用于前一项创建的文档，这并不是用户在更新文档时的真实思维方式。`
- 转换不仅仅出现在协同编辑中。如果你想要一个可以撤销部分更改但保留其他更改的撤销历史（这在协同编辑中很有用，因为你不希望撤销别人的更改，但在其他场景中也会出现），你需要类似的机制。
- **CodeMirror 的变更表示被编码为一系列替换。（插入变为不删除任何内容的替换，删除变为不插入任何内容的替换。）**
- 映射操作在经过**删除**范围时会丢失信息 -> Tombstones
