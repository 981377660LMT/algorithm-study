...
当跨多节点的事务完成时，DBMS 需要询问所有相关节点是否可以安全提交。
这需要特定的协议确保事务的原子性。
...
1. **提前准备投票(Early Prepare Voting)**（不常见）

   - 如果向远程节点发送`你确定是最后执行的`查询，该节点也会在查询结果中返回准备阶段的投票
     最后查询捎带问，结果投票一起回，省趟网络通信费。

2. **准备后提前确认(Early Ack After Prepare)**（常见）
...
- **两阶段提交**：如果协调者在发送准备消息后失败，会`阻塞直到协调者恢复`
