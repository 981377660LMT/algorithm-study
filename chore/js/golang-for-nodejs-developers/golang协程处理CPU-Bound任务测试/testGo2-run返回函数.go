...
func ModSumAsync(start int, end int, mod int, worker *int) int {
	...
	tasks := make([]AsyncFunction, realWorker)
	for i := 0; i < realWorker; i++ {
		tasks[i] = func(...interface{}) Promise {
			return run(i)
		}
	}
...
func PromiseAllInt(funcs ...AsyncFunction) Promise {
	var wg sync.WaitGroup
	res := make([]int, len(funcs))
	promise := make(Promise, 1)

	for fid, f := range funcs {
		wg.Add(1)
		go func(fid int, f AsyncFunction) {
			awaited := <-f() // await
			switch v := awaited.(type) {
			case error:
				promise <- v // reject
