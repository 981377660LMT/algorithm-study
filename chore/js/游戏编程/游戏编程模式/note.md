...
3. 录像与回放系统的实现思路
   ...
   - how：
     ...
     - 五个原则：
       1. Components have no functions ：组件没有行为
       2. Systems have no state：系统没有状态
       3. Shared code lives in utils
       4. Complex side effects should be deferred (例如：长时间的复杂请求（Complex side effects）一般都放到管道里面后台延迟处理)
       5. System can't call other systems
          System 之间不能相互调用（减少耦合）(是由游戏世界（外部框架）来驱动若干 System 的)；System 间的解耦，主要通过事件回调

4. 状态模式、有限状态机 & Unity 版本实现
...
状态模式主要解决的就是`当控制一个对象状态转换的条件表达式过于复杂的情况`，它把状态的判断逻辑转移到表示不同的一系列类当中，可以把复杂的逻辑判断简单化。
