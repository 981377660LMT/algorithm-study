...
- **GC（Garbage Collection）** 是一种由系统统一负责“释放不再使用对象内存”的策略，可大幅减轻研发人员的内存管理负担，常用于高并发项目。由垃圾收集器以类似守护协程的方式在后台运作。
...
## 3. Golang 垃圾回收机制综述 （并发三色标记法+混合写屏障机制）
...
- 但并发会带来新的问题：在标记过程中，用户协程可以修改指针关系，可能出现 **漏标（对象实际可达却变白被回收）** 或 **多标（浮动垃圾）**。
  - **漏标** 是不可接受的错误，因为活对象被误回收会导致程序崩溃。
    ![alt text](image.png)
  - **多标**（或称浮动垃圾） 相对可容忍：一些本该回收的对象被误标为存活，下个 GC 周期再回收即可。
...
### 3.3 解决内存碎片(Golang 垃圾回收如何解决内存碎片问题?)
...
### 3.4 没有使用分代收集(Golang为什么不选择分代垃圾回收机制？)
...
- 保持简单：同时分代 GC 需更多调度和维护开销，以及更复杂的屏障逻辑。Go 核心团队最终没有选择分代策略。
- Go 运行时使用的内存分配算法（类似于 tcmalloc）已经能够很好地控制内存碎片问题。由于不需要额外进行内存整理，Go 也不需要为此引入分代 GC 带来的复杂性。
...
- **强三色不变式**：任何`白色对象都不能直接被黑色对象引用`。一旦出现“黑->白”，必须先把那个白对象标记成灰。破坏 (1)；
- **弱三色不变式**：允许临时出现“黑->白”引用，但必须保证此白对象一定能从某个灰对象路径到达，所以不会真正漏标。间接破坏了（1）、（2）的联动。
...
1. **插入写屏障（Dijkstra Barrier）**
   ...
   - 在“黑对象插入新引用”时，先把目标对象从白色变成灰色，保证不出现新的黑->白链接，实现强三色不变式。
   - 主要解决“新增引用”导致的漏标。
...
2. **删除写屏障（Yuasa Barrier）**
   ...
   - 在删除引用前，把被删指向的白对象标记为灰，这样即使原有路径断开，也能继续追踪到它，实现弱三色不变式。
...
> 栈是协程私有独占的，无法被其他协程改变引用的地址，所以一旦被标记为黑色，就不可能还引用着白色对象
