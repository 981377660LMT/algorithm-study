...
func minOperations(nums []int, queries []int) []int64 {
	...
	for i, v := range queries {
		leftCount, leftSum := wm.CountPrefix(0, len(nums), v, 0)
		rightCount, rightSum := len(nums)-leftCount, sum-leftSum
		res[i] = int64(leftCount*v-leftSum) + int64(rightSum-rightCount*v)
	}
	return res
