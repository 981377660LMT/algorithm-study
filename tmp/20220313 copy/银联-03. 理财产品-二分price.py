...
# 二分得到的答案是零，表明此时所有产品的个数加起来不到 limit 个，直接返回总价值即可（注意取模）。
# 二分得到的答案是正数。此时所有价值不小于答案的产品的个数加起来大于等于 limit 个，我们先求出它们的总和，再减去多算的价值刚好等于答案的产品即可。


# 这道题二分想的是找到最小的价格，使得count<=limit 实际上这样做是不行的，因为最后剩下的不足minPrice
