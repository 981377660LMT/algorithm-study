...
function maximumSumQueries(nums1: number[], nums2: number[], queries: number[][]): number[] {
  ...
  // 倒叙遍历点
  // 线段树维护
  const seg = useAtcoderLazySegmentTree(allY.size + 10, {
    e: () => -INF,
    id: () => -INF,
    op: (a, b) => Math.max(a, b),
  ...
  for (let i = qWithId.length - 1; i >= 0; i--) {
    ...
    const cur = seg.query(rank(qy), allY.size + 5)
...
// [31,92]
// [28,87]
// [[50,87],[16,22],[99,17],[50,21],[10,49],[38,50],[26,37]]
console.log(
  maximumSumQueries(
    [31, 92],
    [28, 87],
