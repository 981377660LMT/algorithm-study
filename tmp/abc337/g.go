...
func main() {
	...
	// 子树内比自己小的节点数
	subSmaller := make([]int, n)
	bit := NewBitArray(n)
	for i := 0; i < n; i++ {
		dfn := tree.LID[i]
		lid, rid := tree.LID[i], tree.RID[i]
		subSmaller[i] = bit.QueryRange(lid, rid)
		bit.Add(dfn, 1)
	}

	smallerPreSum := make([]int, n+1)
	for i := 0; i < n; i++ {
		smallerPreSum[i+1] = smallerPreSum[i] + subSmaller[i]
	}
	fmt.Println(subSmaller)
