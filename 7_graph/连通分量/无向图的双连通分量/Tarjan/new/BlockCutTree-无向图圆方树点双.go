...
func ArticulationPoints() {
	...
	tree := BlockCutTree(graph)
	deg := make([]int, len(tree))
	for i, to := range tree {
		deg[i] = len(to)
	}
...
func yosupo() {
	...
	tree := BlockCutTree(graph)
	fmt.Fprintln(out, len(tree)-n) // block数量=点双连通分量数量
	for i := n; i < len(tree); i++ {
...
func abc334g() {
	...
	for i := 0; i < curId; i++ {
		if len(tree[i]) >= 2 { // !割点
			res = append(res, basePart+len(tree[i])-1)
...
func BlockCutTree(graph [][]int) (tree [][]int) {
	n := int32(len(graph))
	low := make([]int32, n)
	order := make([]int32, n)
