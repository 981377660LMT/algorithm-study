// DynamicShortestPath
// 指定起点终点的动态最短路，带修改的有向图最短路/带修改的最短路问题
...
func demo() {
	...
	m := int32(2e5)
	time1 := time.Now()
	S := NewDirectedModifiableShortestPath(n, m, 0, n-1)
	for i := int32(0); i < m; i++ {
		S.AddEdge(i%n, (i+1)%n, int(i+10000))
		S.AddEdge((i+1)%n, (i+6)%n, int(i+10000))
	}

	for i := int32(0); i < m; i++ {
		S.QueryOnModifyEdge(m-i-1, int(i+10000))
	}
