...
func Amidakuji() {
	...
	fmt.Fscan(in, &n)
	n--
	bf := NewBipartiteFlowAdvanced(n, n)
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			var x int
			fmt.Fscan(in, &x)
...
func ArrangementOfPieces() {
	...
	var n, m int
	fmt.Fscan(in, &n, &m)
	flow := NewBipartiteFlowAdvanced(n, n)
...
func MatchingOnBipartiteGraph() {
	...
	var L, R, M int
	fmt.Fscan(in, &L, &R, &M)
	bf := NewBipartiteFlowAdvanced(L, R)
	for i := 0; i < M; i++ {
		var u, v int
		fmt.Fscan(in, &u, &v)
		bf.AddEdge(u, v)
...
type BipartiteFlowAdvanced struct {
	N, M           int
	MatchL, MatchR []int
	timeStamp      int
	g, rg          [][]int
...
func NewBipartiteFlowAdvanced(n, m int) *BipartiteFlowAdvanced {
...
//	返回(左侧点,右侧点)的匹配对.
//	!0<=左侧点<n,0<=右侧点<m.
func (bf *BipartiteFlowAdvanced) MaxMatching() [][2]int {
	bf.matched = true
	for {
		bf.buildAugmentPath()
		bf.timeStamp++
...
func (bt *BipartiteFlowAdvanced) LexMaxMatching() [][2]int {
	if !bt.matched {
		bt.MaxMatching()
	}
	for _, vs := range bt.g {
...
// 最小点覆盖.
func (bt *BipartiteFlowAdvanced) MinVertexCover() []int {
...
func (bt *BipartiteFlowAdvanced) LexMinVertexCover(ord []int) []int {
...
// 最大独立集.
func (bt *BipartiteFlowAdvanced) MaxIndependentSet() []int {
	visited := bt.findResidualPath()
	res := []int{}
	for i := 0; i < (bt.N + bt.M); i++ {
		if visited[i] != (i >= bt.N) {
...
// 最小边覆盖.
func (bt *BipartiteFlowAdvanced) MinEdgeCover() [][2]int {
	es := bt.MaxMatching()
	for i := 0; i < bt.N; i++ {
		if bt.MatchL[i] >= 0 {
			continue
...
func (bf *BipartiteFlowAdvanced) BuildRisidualGraph() [][]int {
	if !bf.matched {
		bf.MaxMatching()
	}
...
func (bf *BipartiteFlowAdvanced) findResidualPath() []bool {
...
func (bf *BipartiteFlowAdvanced) buildAugmentPath() {
	que := []int{}
...
func (bf *BipartiteFlowAdvanced) findMinDistAugmentPath(a int) bool {
...
func (bf *BipartiteFlowAdvanced) findAugmentPath(a int) bool {
	bf.used[a] = bf.timeStamp
	for _, b := range bf.g[a] {
		c := bf.MatchR[b]
