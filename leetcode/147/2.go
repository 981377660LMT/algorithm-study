...
type TaskManager struct {
	...
	sortedList *SortedList32
}

func Constructor(tasks [][]int) TaskManager {
	lessFunc := func(a, b S) bool {
		ap := decode(a)
		bp := decode(b)
	...
	manager := TaskManager{
		...
		sortedList: NewSortedList32(lessFunc),
	}

	for _, t := range tasks {
		userId, taskId, priority := t[0], t[1], t[2]
		manager.taskInfo[taskId] = &TaskInfo{userId, priority}
		manager.sortedList.Add(encode(priority, taskId))
	}
...
func (this *TaskManager) Add(userId int, taskId int, priority int) {
	this.taskInfo[taskId] = &TaskInfo{userId, priority}
	this.sortedList.Add(encode(priority, taskId))
}

func (this *TaskManager) Edit(taskId int, newPriority int) {
	info := this.taskInfo[taskId]
	oldCode := encode(info.priority, taskId)
	this.sortedList.Discard(oldCode)
	info.priority = newPriority
	newCode := encode(newPriority, taskId)
	this.sortedList.Add(newCode)
}

func (this *TaskManager) Rmv(taskId int) {
	info := this.taskInfo[taskId]
	code := encode(info.priority, taskId)
	this.sortedList.Discard(code)
	delete(this.taskInfo, taskId)
}

func (this *TaskManager) ExecTop() int {
	if this.sortedList.Len() == 0 {
		return -1
	}
	top := this.sortedList.Max()
	pair := decode(top)
	taskId := pair.taskId
	info := this.taskInfo[taskId]
	userId := info.userId

	this.sortedList.Discard(top)
	delete(this.taskInfo, taskId)
	return userId
}
...
func encode(priority, taskId int) S {
	return (int64(priority) << 32) | int64(taskId)
}

func decode(val S) pair {
	p := int((val >> 32) & 0xffffffff)
	t := int(val & 0xffffffff)
	return pair{p, t}
}
...
func (sl *SortedList32) Range(minVal, maxVal S) []S {
	if sl.less(maxVal, minVal) {
		return nil
	}
	res := []S{}
	pos := sl._locBlock(minVal)
	m := int32(len(sl.blocks))
	for i := pos; i < m; i++ {
		block := sl.blocks[i]
		for j := 0; j < len(block); j++ {
