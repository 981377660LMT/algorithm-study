import { SegmentTree01 } from '../../6_tree/线段树/经典题/01线段树/SegmentTree01'
...
function minOperations(nums: number[]): number {
  const seg01 = new SegmentTree01(nums)
  let res = 0
  for (let i = 0; i < nums.length; i++) {
    const cur = seg01.onesCount(i, i + 1) // TODO: Get，Set
    if (cur === 0) {
      res++
      seg01.flip(i, nums.length)
    }
  }
  return res
}
