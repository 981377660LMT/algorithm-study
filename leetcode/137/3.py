...
class Solution:
    def maximumValueSum(self, board: List[List[int]]) -> int:
        ...
        px = [
            LPVar(f"px1", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=ROW - 1),
            LPVar(f"px2", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=ROW - 1),
            LPVar(f"px3", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=ROW - 1),
        ]
        py = [
            LPVar(f"py1", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=COL - 1),
            LPVar(f"py2", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=COL - 1),
            LPVar(f"py3", variable_type=LPVarType.INTEGER, lower_bound=0, upper_bound=COL - 1),
        ]

        model.add_constraint(px[0] != px[1])
        model.add_constraint(px[0] != px[2])
        model.add_constraint(px[1] != px[2])
        model.add_constraint(py[0] != py[1])
        model.add_constraint(py[0] != py[2])
        model.add_constraint(py[1] != py[2])

        model.set_objective(sum(board[px[i].value()][py[i].value()] for i in range(3)))
        model.solve()
        return int(model.objective.value())


# [[-3,1,1,1],[-3,1,-3,1],[-3,2,1,1]]
print(Solution().maximumValueSum(board=[[-3, 1, 1, 1], [-3, 1, -3, 1], [-3, 2, 1, 1]]))
