...
func demo() {
	treap := NewFHQTreap(16)
	treap.Insert(1, 1)
	treap.Insert(2, 2)
	treap.Insert(3, 3)
	treap.Insert(4, 4)
	treap.Insert(5, 40)
	fmt.Println(treap.At(0))
	fmt.Println(treap.At(1))
	fmt.Println(treap.At(2))
	fmt.Println(treap.At(3))
	fmt.Println(treap.At(4))
	fmt.Println(treap.RangeSum(1, 4))
}
...
func (t *FHQTreap) At(index int) int {
	...
	// }
	return t.nodes[t.kthNode(index+1)].data
}

func (t *FHQTreap) Append(data *Data) {
	t.Insert(t.Size(), *data)
}
...
// 区间删除
func (t *FHQTreap) RangePop(left, right int) {
	var x, y, z int
...
// 区间修改
func (t *FHQTreap) RangeAdd(left, right int, data int) {
	var x, y, z int
	t.split(t.root, right, &y, &z)
	t.split(y, left-1, &x, &y)
	t.add(y, &data)
...
// 区间查询
func (t *FHQTreap) RangeSum(left, right int) int {
	var x, y, z int
	t.split(t.root, right, &y, &z)
	t.split(y, left-1, &x, &y)
	res := t.nodes[y].sum
...
// 需要从外界传入add逻辑
func (t *FHQTreap) add(root int, data *Data) {
	node := &t.nodes[root]
	node.lazyAdd += *data
	node.data += *data
	node.sum += *data * node.size
}
...
func (t *FHQTreap) pushDown(root int) {
	...
	if node.lazyAdd != 0 {
		// TODO
		t.add(node.left, &node.lazyAdd)
		t.add(node.right, &node.lazyAdd)
		node.lazyAdd = 0
	}
}
...
func (t *FHQTreap) newNode(data *Data) int {
	...
	if t.recyc != 0 {
		index := t.recyc
		if t.nodes[index].left != 0 && t.nodes[index].right != 0 {
			t.recyc = t.merge(t.nodes[index].left, t.nodes[index].right)
		} else if t.nodes[index].left != 0 {
			t.recyc = t.nodes[index].left
		} else {
			t.recyc = t.nodes[index].right
		}
