...
def dfs(
    index: int, nums: List[int], preState: int, curState: int, curCost: int,
    ...
    if nums[index] == 1:
        ...
        # 这个位置横向连接，看前一个是否横向连接
        isLeftHorizontal = (
            index > 0 and nums[index - 1] == 1 and not ((curState >> (index - 1)) & 1)
        )

        yield from dfs(
            index + 1, nums, preState, curState, curCost + (0 if isLeftHorizontal else 1)
        )
    else:
        yield from dfs(index + 1, nums, preState, curState, curCost)
...
startStates = dfs(0, matrix[0], 0, 0, 0)
for state, cost in startStates:
    dp[0][state] = cost

for r in range(1, row):
    for preState, preCost in dp[r - 1].items():
        # 这里需要改一下，应该在外面预处理出每行的state，里面再根据preState计算cost
        curStates = dfs(0, matrix[r], preState, 0, 0)
