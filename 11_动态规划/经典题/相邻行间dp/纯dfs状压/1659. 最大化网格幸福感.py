...
class Solution:
    def getMaxGridHappiness(
        ...
        def dfs(row: int, col: int, intro: int, extro: int, state: Tuple[int]) -> int:
            ...
            if col == n:
                return dfs(row + 1, 0, intro, extro, state)
            if row == m:
                return 0

            # 不填
            res = dfs(row, col + 1, intro, extro, state[1:] + (0,))
            ...
            if intro:
                ...
                if col:
                    if state[-1] == 1:
                        ...
                        score -= 10
                res = max(res, score + dfs(row, col + 1, intro - 1, extro, state[1:] + (1,)))
            ...
            if extro:
                ...
                if col:
                    if state[-1] == 1:
                        ...
                        score += 40
                res = max(res, score + dfs(row, col + 1, intro, extro - 1, state[1:] + (2,)))
