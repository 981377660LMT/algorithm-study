...
func main() {
	...
	initRow := make([]int, ROW)
	for i := 0; i < ROW; i++ {
		initRow[i] = i
	}
	rowTreap := NewFHQTreap(initRow)
	initCol := make([]int, COL)
	for i := 0; i < COL; i++ {
		initCol[i] = i
	}
	colTreap := NewFHQTreap(initCol)
	var q int
	fmt.Fscan(in, &q)
	for i := 0; i < q; i++ {
		var a, b int // [1,n]
		...
		rowTreap.Reverse(0, a)
		rowTreap.Reverse(a, ROW)
		colTreap.Reverse(0, b)
		colTreap.Reverse(b, COL)
	}
	...
	for i := 0; i < ROW; i++ {
		for j := 0; j < COL; j++ {
			res[rowTreap.At(i)][colTreap.At(j)] = string(grid[i][j])
		}
	}
