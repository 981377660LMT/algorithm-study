...
def solve1(intervals: List[Tuple[int, int]], queries: List[Tuple[int, int]]) -> List[int]:
    """
    !O(upper*q) 容斥原理+区间dp
    dfs[left][right]表示闭区间[left,right]内的完整闭区间的个数
    """
    ...
    def dfs(L: int, R: int) -> int:
        if L > R:
            return 0
        return dfs(L + 1, R) + dfs(L, R - 1) - dfs(L + 1, R - 1) + counter[(L, R)]

    counter = Counter(intervals)
    return [dfs(left, right) for left, right in queries]
