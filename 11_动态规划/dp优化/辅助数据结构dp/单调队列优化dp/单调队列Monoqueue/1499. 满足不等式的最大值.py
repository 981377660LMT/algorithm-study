...
# 每次搜索到一个新的 point，我们可以通过单调队列，直接取出之前的点中，y - x 的最大值。
# 不过，每次取出元素前，我们需要检验一下，这个元素的 x 值是否满足限制条件。


# 总结:
# lessons learned: when using a heap to maintain an ordered subarray,
# consider using a monotonic stack.
# !当使用堆来维护有序的子数组时，考虑使用单调队列。
