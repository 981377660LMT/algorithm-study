// Api:
//
// NewRangeAddMakeDecreasing(n int32, f func(i int32) int) *RangeAddRangeMakeDecreasing
// Get(i int32) int
// GetAll() []int
// Set(i int32, x int)
// RangeAdd(start, end int32, x int)
// RangeAssign(start, end int32, x int)
// !MakeDecreasing(start, end int32) -> 从start到end的区间变成非严格单调递减.从前缀开始修正.
...
func demo() {
	{
		A := []int{1, 2, 3, 4, 5}
		seg := NewRangeAddMakeDecreasing(int32(len(A)), func(i int32) int { return A[i] })
		...
		fmt.Println(seg.GetAll())
		seg.Set(1, 10)
		seg.MakeDecreasing(1, 3)
		fmt.Println(seg.GetAll())
		seg.MakeDecreasing(0, 5)
		fmt.Println(seg.GetAll())
	}

	{
		A := []int{3, 1, 4}
		seg := NewRangeAddMakeDecreasing(int32(len(A)), func(i int32) int { return A[i] })
		seg.MakeDecreasing(0, 3)
		fmt.Println(seg.GetAll())
	}
}

// SegmentTreeRangeAddRangeDecreasing
type RangeAddRangeMakeDecreasing struct {
	n      int32
	s1, s2 *fastSet32
	seg    *segmentTreeDual32
}

func NewRangeAddMakeDecreasing(n int32, f func(i int32) int) *RangeAddRangeMakeDecreasing {
	...
	return &RangeAddRangeMakeDecreasing{n: n, s1: s1, s2: s2, seg: seg}
}

func (r *RangeAddRangeMakeDecreasing) Get(i int32) int {
	return r.seg.Get(r.s1.Prev(i))
}

func (r *RangeAddRangeMakeDecreasing) GetAll() []int {
	res := r.seg.GetAll()
	p := int32(0)
...
func (r *RangeAddRangeMakeDecreasing) Set(i int32, x int) {
	r.split(i)
	r.split(i + 1)
	r.seg.Set(i, x)
	r.s2.Insert(i)
...
func (r *RangeAddRangeMakeDecreasing) RangeAdd(start, end int32, x int) {
	r.split(start)
	r.split(end)
	if x > 0 {
		r.s2.Insert(start)
...
func (r *RangeAddRangeMakeDecreasing) RangeAssign(start, end int32, x int) {
	r.split(start)
	r.split(end)
	r.s1.Insert(start)
	r.s1.Insert(end)
...
func (r *RangeAddRangeMakeDecreasing) MakeDecreasing(start, end int32) {
	r.split(start)
	r.split(end)
...
func (r *RangeAddRangeMakeDecreasing) split(p int32) {
