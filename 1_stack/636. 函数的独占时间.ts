 ...
 * 函数的 独占时间 定义是`在这个函数在程序所有函数调用中执行时间的总和，`
...
const exclusiveTime = function (n: number, logs: string[]): number[] {
  ...
  for (const log of logs) {
    ...
    const time = Number(details[2])

    if (state === 'start') {
      // 栈顶函数不被调用地执行time-pre时间
      if (stack.length > 0) res[stack[stack.length - 1]] += time - pre
      stack.push(id)
      pre = time
    } else {
      // 栈顶函数不被调用地执行time-pre+1时间
