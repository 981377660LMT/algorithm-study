...
func main() {
	...
	for i := 0; i < N; i++ {
		ws[i], vs[i] = io.NextInt(), io.NextInt()
	}

	funcs := make([]SlopeFunc, N)
	for i := 0; i < N; i++ {
		funcs[i] = NewQuadratic(-vs[i], ws[i], 0, 0, W)
	}
	coeff := make([]int, N)
	for i := 0; i < N; i++ {
		coeff[i] = -1
	...
	res := 0
	for targetW := 0; targetW <= W; targetW++ {
		minimum, _, ok := MinConvexSumUnderLinearConstraint(coeff, funcs, targetW)
		if !ok {
			continue
		}
		if res < minimum {
