// abc377-D - Many Segments 2-二维线段树
...
func main() {
	...
	for i := int32(0); i < N; i++ {
		L[i], R[i] = int32(io.NextInt()), int32(io.NextInt())
	}

	seg := NewSegmentTree2DSparse32Fast(N, func(i int32) (int32, int32, int32) {
		return L[i], R[i], R[i]
	}, true)

	res := 0
	for l := int32(1); l <= M; l++ {
		minY := seg.Query(l, M+1, 0, M+1)
		if minY == INF32 {
			minY = M + 1
		}
		res += int(minY - l)
...
func op(a, b E) E { return min32(a, b) }
