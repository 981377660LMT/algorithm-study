...
if __name__ == "__main__":
    for _ in range(1):
        N = 4
        A = [randint(1, 99) for _ in range(1 << N)]

        L, R = randint(0, 1 << N - 1), randint(0, 1 << N - 1)
        if L > R:
            L, R = R, L
        ...
        def divide(start: int, end: int) -> List[int]:
            ...
            while start < end:
                if start & 1:
                    leftSegments.append(start)
                    start += 1
                if end & 1:
                    end -= 1
                    rightSegments.append(end)
                start >>= 1
                end >>= 1
            return leftSegments + rightSegments[::-1]

        def query(a: int, b: int) -> int:
            l = 2**a * b
            r = 2**a * (b + 1) - 1
            return sum(A[l : r + 1]) % 100

        def output(res: int) -> None:
            if res != sum(A[L : R + 1]) % 100:
                print(N, L, R)
                print(A)
                print(res, sum(A[L : R + 1]) % 100)
                exit()
        ...
        res = []
        intervals = divide(L, R + 1)
        print(L, R, 988)
        for v in intervals:
            print(leftLeaf(v), rightLeaf(v))
