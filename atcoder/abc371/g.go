...
//
// 得られる
// A としてありえるもののうち、辞書順で最小のものを出力してください。
...
func main() {
	...
	G := NewFunctionalGraph32(N)
	for i := int32(0); i < N; i++ {
		G.AddDirectedEdge(i, P[i], 1)
	}
	G.Build()
	...
	for i := int32(0); i < N; i++ {
		...
		path := []int32{}
		min_, minIdx := N+1, int32(-1)
		curVisited := make(map[int32]struct{})
		cur := i
		for {
			if _, ok := curVisited[cur]; ok {
				break
			}
			curVisited[cur] = struct{}{}
			path = append(path, cur)
			if A[cur] < min_ {
				min_, minIdx = A[cur], cur
			}
			cur = G.To[cur]
		}

		base = G.Dist(i, minIdx, false)
		if !G.InCycle(minIdx) {
			perm := G.JumpAll(base)
			for i := int32(0); i < N; i++ {
				res[perm[i]] = A[i]
			...
			cycle := G.CollectCycle(G.Root[i])
			mod = len(cycle)
		}

		belong := G.Root[i]
		for _, v := range groups[belong] {
			visited[v] = true
		}
	}
