...
class SegmentLazyQuadraticDivision(Generic[T, F]):
    ...
    def __init__(
        ...
        self.data = [
            a[k * self.bucket_size : (k + 1) * self.bucket_size] for k in range(self.bucket_cnt)
        ]
        self.bucket_sum = [reduce(self.op, v) for v in self.data]
    ...
    def apply(self, l: int, r: int, f: F) -> None:
        ...
        def _change_data(k: int, l: int, r: int) -> None:
            ...
            self.bucket_sum[k] = reduce(self.op, self.data[k])
        ...
        if k1 == k2:
            ...
            if k1 < self.bucket_cnt:
                if l == 0:
                    ...
                    self.bucket_sum[k1] = self.mapping(f, self.bucket_sum[k1])
                else:
            ...
            for i in range(k1 + 1, k2):
                ...
                self.bucket_sum[i] = self.mapping(f, self.bucket_sum[i])
            if k2 < self.bucket_cnt:
                if r == len(self.data[k2]):
                    ...
                    self.bucket_sum[k2] = self.mapping(f, self.bucket_sum[k2])
                else:
                    _change_data(k2, 0, r)
        ...
        if k1 == k2:
            ...
            if l < len(self.data[k1]):
                ...
                if self.bucket_lazy[k1] != self.id:
                    s = self.mapping(self.bucket_lazy[k1], s)
            if k1 + 1 < k2:
                s = reduce(self.op, self.bucket_sum[k1 + 1 : k2], s)
    ...
    def all_prod(self) -> T:
        ...
        :math:`O(\\sqrt{n})` です。
        """
        return reduce(self.op, self.bucket_sum)
    ...
    def __getitem__(self, k: int) -> T:
        ...
        return (
            ...
            else self.mapping(self.bucket_lazy[p], self.data[p][k - p * self.bucket_size])
        )
    ...
    def __setitem__(self, k, v):
        ...
        self.bucket_sum[p] = reduce(self.op, self.data[p])
