...
func demo() {
	...
	wm := NewWaveletMatrixWithSumOld(nums, nums, -1, true)
	fmt.Println(wm.CountRange(0, 3, 3, 7, 0))
	fmt.Println(wm.KthValueAndSum(1, 5, 0, 0))
	nums = []int{1, 2, 3, 4, 5}
	wm = NewWaveletMatrixWithSumOld(nums, nums, -1, false)
...
func yuki919() {
	...
	wm := NewWaveletMatrixWithSumOld(nums, nil, -1, true)
...
func yuki1332() {
	...
	M := NewWaveletMatrixWithSumOld(nums, nil, -1, true)
	var q int
	fmt.Fscan(in, &q)
	for i := 0; i < q; i++ {
		var start, end int32
...
func yuki2065() {
	...
	M := NewWaveletMatrixWithSumOld(nums, nums, -1, true)
	for i := int32(0); i < q; i++ {
		var start, end int32
		var x int
		fmt.Fscan(in, &start, &end, &x)
...
func (*WaveletMatrixWithSumOld) e() WmSum            { return 0 }
func (*WaveletMatrixWithSumOld) op(a, b WmSum) WmSum { return a + b }
func (*WaveletMatrixWithSumOld) inv(a WmSum) WmSum   { return -a }

type WaveletMatrixWithSumOld struct {
	n, log   int32
	mid      []int32
	bv       []*BitVector
	key      []WmValue
...
func NewWaveletMatrixWithSumOld(nums []WmValue, sumData []WmSum, log int32, compress bool) *WaveletMatrixWithSumOld {
	wm := &WaveletMatrixWithSumOld{}
	wm.build(nums, sumData, log, compress)
	return wm
}

func (wm *WaveletMatrixWithSumOld) build(nums []WmValue, sumData []WmSum, log int32, compress bool) {
...
func (wm *WaveletMatrixWithSumOld) CountRange(start, end int32, a, b WmValue, xorVal WmValue) int32 {
	return wm.CountPrefix(start, end, b, xorVal) - wm.CountPrefix(start, end, a, xorVal)
}

func (wm *WaveletMatrixWithSumOld) SumRange(start, end, k1, k2 int32, xorVal WmValue) WmSum {
	if k1 < 0 {
		k1 = 0
	}
	if k2 > end-start {
...
func (wm *WaveletMatrixWithSumOld) CountPrefix(start, end int32, x WmValue, xor WmValue) int32 {
	if xor != 0 {
		if !wm.setLog {
...
func (wm *WaveletMatrixWithSumOld) SumPrefix(start, end, k int32, xor WmValue) WmSum {
	_, sum := wm.KthValueAndSum(start, end, k, xor)
	return sum
}

// [start, end)区间内第k(k>=0)小的元素.
func (wm *WaveletMatrixWithSumOld) Kth(start, end, k int32, xorVal WmValue) WmValue {
	if xorVal != 0 {
		if !wm.setLog {
...
func (wm *WaveletMatrixWithSumOld) KthValueAndSum(start, end, k int32, xorVal WmValue) (WmValue, WmSum) {
	if start >= end {
		return -1, wm.e()
	}
	if k >= end-start {
...
func (wm *WaveletMatrixWithSumOld) Median(start, end int32, upper bool, xorVal WmValue) WmValue {
	n := end - start
	var k int32
	if upper {
		k = n / 2
...
func (wm *WaveletMatrixWithSumOld) SumAll(start, end int32) WmSum {
	return wm._get(wm.log, start, end)
}

// 使得predicate(count, sum)为true的最大的(count, sum).
func (wm *WaveletMatrixWithSumOld) MaxRight(predicate func(int32, WmSum) bool, start, end int32, xorVal WmValue) (int32, WmSum) {
	if xorVal != 0 {
		if !wm.setLog {
			panic("log should be set when xor is used")
		}
...
//
//	如果不存在则返回-INF
func (wm *WaveletMatrixWithSumOld) Floor(start, end int32, x WmValue, xor WmValue) WmValue {
...
//
//	如果不存在则返回INF
func (wm *WaveletMatrixWithSumOld) Ceil(start, end int32, x WmValue, xor WmValue) int {
	less := wm.CountPrefix(start, end, x, xor)
	if less == end-start {
		return INF
	}
...
func (wm *WaveletMatrixWithSumOld) _get(d, l, r int32) WmSum {
	return wm.op(wm.inv(wm.presum[d][l]), wm.presum[d][r])
}

func (wm *WaveletMatrixWithSumOld) _argSort(nums []WmValue) []int32 {
...
func (wm *WaveletMatrixWithSumOld) _maxs(nums []WmValue) WmValue {
	res := nums[0]
	for _, v := range nums {
		if v > res {
			res = v
...
func (wm *WaveletMatrixWithSumOld) _lowerBound(nums []WmValue, target WmValue) WmValue {
...
func (wm *WaveletMatrixWithSumOld) _binarySearch(f func(int32) bool, ok, ng int32) int32 {
	for abs32(ok-ng) > 1 {
		x := (ok + ng) >> 1
		if f(x) {
			ok = x
