// Ex - Range Harvest Query -种菜模型
// !给定一片N格的农田，从第0天开始，第i格农田每天会长出i的作物(i>=1)。
// 给出Q个询问，每次询问以D，L，R的格式给出，
// 表示询问在第D天，收割[L，R]的农田，会获得多少作物？
// 答案对998244353取模.注意询问相互依赖，即在每一次收割后，[L,R]的作物应当清零.
// 1<=n<=1e18
// 1<=q<=2e5
// 1<=d1<d2<...<dq<=1e18
// 1<=l<=r<=n
...
// 1. 线段树
// 2. 珂朵莉树
// https://zhuanlan.zhihu.com/p/528328536
...
func main() {
	...
	for i := 0; i < q; i++ {
		var day, left, right int
		fmt.Fscan(in, &day, &left, &right)
		operations[i] = [3]int{day, left, right}
	}

	res := RangeHarvestQuery(n, operations)
	for _, v := range res {
		fmt.Fprintln(out, v)
	}
}

// 每割一次区间[left,right],都会进行区间赋值操作
// 对于区间[left,right]中last[i]相同的子区间[a,b],和为 sum(i*(day-last[i]))
// 也即 (day-last[i])*(left+right)*(right-left+1)/2
func RangeHarvestQuery(n int, operations [][3]int) []int {
	last := NewIntervals(-1, 1, n+1) // 上一次的收割日期
	last.Set(1, n+1, 0)

	res := make([]int, len(operations))
	for i, op := range operations {
		day, L, R := op[0], op[1], op[2]
		curSum := 0
		last.EnumerateRange(L, R+1, func(start, end int, value int) {
			left, right := start, end-1
			dayRange := (day - value) % MOD
			tmp1 := (left + right) % MOD
			tmp2 := (right - left + 1) % MOD
			curSum += dayRange * tmp1 % MOD * tmp2 % MOD * INV2 % MOD
			curSum %= MOD
		}, false)
