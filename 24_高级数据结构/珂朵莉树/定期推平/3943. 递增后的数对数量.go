// 22_专题\离线查询\根号分治\3943. 递增后的数对数量.go
// 普通分块被卡哈希了，换一种思路
...
func numberOfPairs(nums1 []int, nums2 []int, queries [][]int) []int {
	...
	nums1Count := map[int]int{}
	for _, v := range nums1 {
		nums1Count[v]++
	}

	limit := int32(200)
	valueToIndexes := map[int][]int32{}
	rebuild := func() {
		valueToIndexes = map[int][]int32{}
		odt.EnumerateAll(func(start, end int32, add *int) {
			for i := start; i < end; i++ {
				nums2[i] += *add
				valueToIndexes[nums2[i]] = append(valueToIndexes[nums2[i]], i)
			}
		})
		odt.Set(0, n, 0)
	}
	...
	update := func(left, right int32, add int) {
		if odt.Len > limit {
			rebuild()
		}
		odt.Split(left)
		odt.Split(right)
		odt.EnumerateRange(left, right, func(start, end int32, value *int) {
			*value += add
		}, false)
	}

	_query := func(target int) int {
		res := 0
		odt.EnumerateAll(func(start, end int32, add *int) {
			positions := valueToIndexes[target-*add]
			left := lowerBound32(positions, start)
			right := lowerBound32(positions, end)
			res += right - left
		})
		return res
	}
	query := func(total int) int {
		res := 0
		for value, count := range nums1Count {
			res += count * _query(total-value)
		}
		return res
	}
	...
	for _, q := range queries {
		if q[0] == 1 {
			...
			right := int32(q[2] + 1)
			add := q[3]
			update(left, right, add)
		} else {
			res = append(res, query(q[1]))
		}
	}
	return res
}

func lowerBound32(nums []int32, target int32) int {
