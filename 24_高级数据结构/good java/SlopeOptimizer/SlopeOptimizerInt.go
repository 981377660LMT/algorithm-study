...
func main() {
	S := NewSlopeOptimizerInt(16, false, true)
...
// 维护凸包，常用于斜率优化.
type SlopeOptimizerInt struct {
	deque             *Deque
	upperConvexHull   bool // 是否维护上凸包
	maximizeYForSameX bool // 当x相同时，是否取y更大的点
}

func NewSlopeOptimizerInt(initCapacity int, upperConvexHull bool, maximizeYForSameX bool) *SlopeOptimizerInt {
	initCapacity = max(16, initCapacity)
	return &SlopeOptimizerInt{
		deque:             NewDeque2(initCapacity),
		upperConvexHull:   upperConvexHull,
		maximizeYForSameX: maximizeYForSameX,
	}
}

// id需要按照递增顺序加入.
func (so *SlopeOptimizerInt) Add(x, y int, id int32) *Point {
	for !so.deque.Empty() {
		last := so.deque.Back()
		if last.x != x {
			break
		}
		if (last.y >= y) == so.maximizeYForSameX {
			return nil
		} else {
			so.deque.Pop()
		}
...
// 保留下凸包中id大于等于给定值的点.
func (so *SlopeOptimizerInt) Since(id int32) {
	for !so.deque.Empty() && so.deque.Front().id < id {
		so.deque.PopLeft()
	}
}

func (so *SlopeOptimizerInt) GetBestChoice(s float64) int32 {
	if so.upperConvexHull {
		for so.deque.Len() >= 2 {
			h1 := so.deque.PopLeft()
			h2 := so.deque.Front()
...
func (so *SlopeOptimizerInt) Empty() bool { return so.deque.Empty() }
func (so *SlopeOptimizerInt) Clear()      { so.deque.Clear() }
func (so *SlopeOptimizerInt) Len() int    { return so.deque.Len() }

func (so *SlopeOptimizerInt) _slope(a, b *Point) float64 {
	if b.x == a.x {
		if b.y == a.y {
			return 0
		} else if b.y > a.y {
