...
public class PersistentLeftistTree<K> extends CloneSupportObject<PersistentLeftistTree<K>> {
  ...
  public static <K> PersistentLeftistTree<K> createFromCollection(
      Collection<PersistentLeftistTree<K>> trees, Comparator<K> cmp) {
    return createFromDeque(new ArrayDeque<>(trees), cmp);
  }

  public static <K> PersistentLeftistTree<K> createFromDeque(Deque<PersistentLeftistTree<K>> deque,
      Comparator<K> cmp) {
    while (deque.size() > 1) {
  ...
  public static <K> PersistentLeftistTree<K> merge(PersistentLeftistTree<K> a,
      PersistentLeftistTree<K> b, Comparator<K> cmp) {
    if (a == NIL) {
      return b;
    } else if (b == NIL) {
      return a;
  ...
  private static class PersistentLeftistTreeIteratorAdapter<V> implements Iterator<V> {
    ...
    public PersistentLeftistTreeIteratorAdapter(PersistentLeftistTree<V> tree,
  ...
  public static <V> Iterator<V> asIterator(PersistentLeftistTree<V> heap,
      Comparator<V> comparator) {
