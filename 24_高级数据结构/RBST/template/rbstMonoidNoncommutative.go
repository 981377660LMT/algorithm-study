...
func NewRBSTMonoidNoncommutative(persistent bool) *RBSTMonoidNoncommutative {
...
func (rbst *RBSTMonoidNoncommutative) _update(c *node) {
	...
	if left := c.l; left != nil {
		...
		c.revSum = op(c.revSum, left.revSum)
	}
	if right := c.r; right != nil {
		c.size += right.size
		c.sum = op(c.sum, right.sum)
		c.revSum = op(right.revSum, c.revSum)
	}
}
...
func test() {
	for i := 0; i < 10; i++ {
		...
		rbst := NewRBSTMonoidNoncommutative(true)
	...
	rbst := NewRBSTMonoidNoncommutative(false)
...
func testTime() {
	...
	fmt.Println("Time1:", time.Since(time1)) // Time1: 770.543583ms
}
