...
func testTime() {
	...
	root := tree.Build(uint32(n), func(i uint32) E { return E(arr[i]) })

	time1 := time.Now()
	for j := 0; j < int(2e5); j++ {
		root = tree.Set(root, uint32(j), E(j))
		tree.Get(root, uint32(j))
		root = tree.Insert(root, uint32(j), E(j))
		root, _ = tree.Pop(root, uint32(j))
		root = tree.Reverse(root, uint32(j), uint32(n))
		root = tree.ReverseAll(root)
		tree.QueryRange(root, uint32(j), uint32(n))
		tree.QueryAll(root)
		a, b := tree.Split(root, uint32(j))
		root = tree.Merge(a, b)
	}

	fmt.Println(time.Since(time1)) // 612.404542ms
}
