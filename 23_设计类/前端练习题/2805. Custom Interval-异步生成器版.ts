...
import { RecursiveAsyncGenerator, bootStrapAsync } from './bootstrap'
...
function customInterval(fn: () => void, delay: number, period: number): number {
  ...
  bootStrapAsync(_run(id, fn, delay, period))
  return id
}
...
async function* _run(
  ...
  period: number,
  count = 0
): RecursiveAsyncGenerator<void> {
  await sleep(delay + period * count)
  if (!alivedInterval.has(id)) return
  fn()
  yield* _run(id, fn, delay, period, count + 1)
}
