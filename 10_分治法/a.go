...
func (this *StockSpanner) Next(price int) int {
	pos := this.ptr
	this.ptr++
	this.tree.Set(pos, E{price, price})

	cur := this.tree.Get(pos).max
	cand := this.tree.MinLeft(pos, func(e E) bool { return e.max <= cur }) - 1
	if cand == -1 {
		return pos + 1
	}
	return pos - cand
...
type E = struct{ max, min int }
type Id = int

func (*LazySegTree) e() E   { return E{max: -INF, min: INF} }
func (*LazySegTree) id() Id { return 0 }
func (*LazySegTree) op(left, right E) E {
	return E{max(left.max, right.max), min(left.min, right.min)}
}
func (*LazySegTree) mapping(f Id, g E) E {
	return E{f + g.max, f + g.min}
}
