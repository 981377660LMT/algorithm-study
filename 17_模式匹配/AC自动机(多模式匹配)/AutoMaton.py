...
class AhoCorasick:
    ...
    def __init__(self, patterns: List[str]):
        ...
        self._children = [{}]
        self._match = [[]]  # match[i] 表示节点(状态)i对应的字符串在patterns中的下标
        ...
        self._fail = [0] * len(self._children)  # 0 表示虚拟根节点
        self._buildFail()
    ...
    def _buildFail(self) -> None:
        ...
        while queue:
            ...
            for char, child in children[cur].items():
                fail[child] = self._move(fafail, char)
                match[child].extend(match[fail[child]])
                queue.append(child)

    def _move(self, pos: int, char: str) -> int:
        """沿着失配链上跳,找到一个节点fafail,具有char的子节点"""
        while True:
