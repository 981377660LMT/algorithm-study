...
func main() {
	...
	var s, t string
	fmt.Fscan(in, &s, &t)
	tree := NewPalindromicTree(s + "><" + t)
	dps := make([]int, tree.Size())
	dpt := make([]int, tree.Size())
	for i := 0; i < tree.Size(); i++ {
		for _, j := range tree.Nodes[i].Indexes { // 回文出现位置
			if j < len(s) {
				dps[i]++
			} else if j >= len(s)+2 {
				dpt[i]++
	...
	for i := tree.Size() - 1; i > 1; i-- {
		res += dps[i] * dpt[i]
		dps[tree.Nodes[i].Fail] += dps[i]
		dpt[tree.Nodes[i].Fail] += dpt[i]
	}
