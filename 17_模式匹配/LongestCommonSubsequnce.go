...
func LongestCommonSubsequence(s1, s2 Str) int {
	...
	for i := 0; i < len(s1); i++ {
		for j := n - 1; j >= 0; j-- {
			if s1[i] == s2[j] {
				dp[j+1] = max32(dp[j+1], dp[j]+1)
			}
		}
		for j := 0; j < n; j++ {
			dp[j+1] = max32(dp[j+1], dp[j])
		}
	}
	return int(dp[n])
...
func LongestCommonSubsequenceRestore(s1, s2 Str) [][2]int {
	n, m := len(s1), len(s2)
	dp := make([][]int32, n+1)
	for i := range dp {
		dp[i] = make([]int32, m+1)
	...
	for i := 0; i < n; i++ {
		pre := dp[i]
		cur := append(pre[:0:0], pre...)
		dp[i+1] = cur
		for j := 0; j < m; j++ {
			cur[j+1] = max32(cur[j+1], cur[j])
			if s1[i] == s2[j] {
				cur[j+1] = max32(cur[j+1], pre[j]+1)
			}
		}
	}
	...
	for dp[ptr1][ptr2] > 0 {
		if dp[ptr1][ptr2] == dp[ptr1-1][ptr2] {
			...
			ptr1--
			ptr2--
			res = append(res, [2]int{ptr1, ptr2})
		}
	}
