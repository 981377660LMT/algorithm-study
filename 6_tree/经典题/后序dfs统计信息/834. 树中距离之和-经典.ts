...
const sumOfDistancesInTree = (n: number, edges: number[][]): number[] => {
  ...
  function postDFSForDepthAndCount(cur: number, pre: number) {
    for (const next of adjList[cur]) {
      if (next === pre) continue
      depth[next] = depth[cur] + 1
      // 自底向上后序
      postDFSForDepthAndCount(next, cur)
      count[cur] += count[next]
    }
  }

  function preDFSForAnswer(cur: number, pre: number) {
    for (const next of adjList[cur]) {
      if (next === pre) continue
      // 自顶向下先序
