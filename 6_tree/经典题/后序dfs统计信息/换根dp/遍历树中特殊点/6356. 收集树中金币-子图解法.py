...
# !如果可以不回到出发点,那么就是最终的子图的边数(边权和)*2-到最远点的距离。
...
class Solution:
    def collectTheCoins(self, coins: List[int], edges: List[List[int]]) -> int:
        ...
        maxDist = getDistMaxToSpecials(n, edges, specials)  # 注意这里是原树
...
def getDistMaxToSpecials(n: int, edges, specials: List[int]) -> List[int]:
