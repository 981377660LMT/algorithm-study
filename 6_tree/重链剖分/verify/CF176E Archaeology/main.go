...
func main() {
	...
	for i := int32(1); i < n; i++ {
		...
		x--
		y--
		tree.AddEdge(x, y, int(z))
	}
	tree.Build(0)

	res := 0
	dfnSet := NewFastSet32(n)
	...
	add := func(v int32) {
		...
		next := dfnSet.Next(d + 1)
		if next == n {
			next = dfnSet.Next(0)
		}

		u, w := tree.IdToNode[pre], tree.IdToNode[next]
		res += tree.DistWeighted(u, v) + tree.DistWeighted(v, w) - tree.DistWeighted(u, w)
	}
...
func (t *Tree32) LcaRooted(u, v, w int32) int32 {
	return t.Lca(u, v) ^ t.Lca(u, w) ^ t.Lca(v, w)
}
