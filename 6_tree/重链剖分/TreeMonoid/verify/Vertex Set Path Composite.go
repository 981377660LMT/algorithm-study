...
// 0 root mul add: 将结点root的值修改为 f=mul*x+add
// 1 from to x : 按照路径顺序合并 from 到 to 的函数，查询f(x)的值
...
func main() {
	...
	tm := NewTreeMonoid(tree, fs, true)
	for i := 0; i < q; i++ {
		var op int
		fmt.Fscan(in, &op)
		if op == 0 {
			var v, mul, add int
			fmt.Fscan(in, &v, &mul, &add)
			tm.Set(v, E{mul % MOD, add % MOD})
		} else {
			var u, v, x int
			fmt.Fscan(in, &u, &v, &x)
			f := tm.QueryPath(u, v)
			fmt.Fprintln(out, (f.mul*x+f.add)%MOD)
		}
	}
}
...
func NewTreeMonoid(tree *Tree, data []E, isVertex bool) *TreeMonoid {
	...
	res.seg = NewSegmentTree(leaves, e, op)
	if !IS_COMMUTATIVE {
		res.segR = NewSegmentTree(leaves, e, func(e1, e2 E) E { return op(e2, e1) }) // opRev
	}
	return res
}
...
func NewSegmentTree(leaves []E, e func() E, op func(E, E) E) *SegmentTree {
	res := &SegmentTree{e: e, op: op, unit: e()}
	n := len(leaves)
	size := 1
	for size < n {
		size <<= 1
