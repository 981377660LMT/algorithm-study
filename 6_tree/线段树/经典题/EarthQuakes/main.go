...
// 线段树二分，logn 地找到下一个需要操作的点/区间.
// 复杂度由势能保证(注意不是势能线段树，但还是记在这里).
...
func main() {
	...
	seg := NewSegmentTree(n, func(i int) E { return INF })

	update := func(pos, h int) {
		seg.Set(pos, h)
	}

	query := func(start, end, p int) int {
		res := 0
		ptr := start
		for ptr < end {
			ptr = seg.MaxRight(ptr, func(e E) bool { return e > p })
			if ptr >= end {
				break
			}
			seg.Set(ptr, seg.e())
