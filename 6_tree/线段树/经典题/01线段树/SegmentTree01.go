...
// 1 left right: 询问区间 [left,right] 中所有数的和
// 2 left right x: 将区间 [left,right] 中所有数都异或上 x
// !nums[i],x <= 1e6
func CF242E() {
	...
	LOG := 20
	seg := make([]*SegmentTree01, LOG)
	for i := range seg {
		bits := make([]int, n)
		for j := 0; j < n; j++ {
			bits[j] = (nums[j] >> i) & 1
		}
		seg[i] = NewSegmentTree01(bits)
	}

	query := func(start, end int) int {
		res := 0
		for i := 0; i < LOG; i++ {
			ones := seg[i].OnesCount(start, end)
			res += ones * (1 << i)
		}
		return res
	}

	update := func(start, end, x int) {
		for i := 0; i < LOG; i++ {
			if (x>>i)&1 == 1 {
				seg[i].Flip(start, end)
			}
		}
	}
