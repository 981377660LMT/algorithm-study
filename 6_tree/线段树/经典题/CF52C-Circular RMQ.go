...
func main() {
	nums := []int{1, 2, 3, 4, 5}
	crmq := NewCircularRMQFrom(nums)
	fmt.Println(crmq.Query(4, 1)) // 1
}

type CircularRMQ struct {
	seg *LazySegTree
}

// 环形数组RMQ.
func NewCircularRMQ(n int, f func(int) E) *CircularRMQ {
	return &CircularRMQ{seg: NewLazySegTree(n, f)}
}

func NewCircularRMQFrom(circularNums []E) *CircularRMQ {
	return NewCircularRMQ(len(circularNums), func(i int) E { return circularNums[i] })
}

func (c *CircularRMQ) Update(start, end, f Id) {
	if start < end {
		c.seg.Update(start, end, f)
	} else {
		c.seg.Update(start, c.seg.n, f)
		c.seg.Update(0, end, f)
	}
}

func (c *CircularRMQ) Query(start, end int) E {
	if start < end {
		return c.seg.Query(start, end)
	} else {
		return c.seg.op(c.seg.Query(start, c.seg.n), c.seg.Query(0, end))
	}
}
