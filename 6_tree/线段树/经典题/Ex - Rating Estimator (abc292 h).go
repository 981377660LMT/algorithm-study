// 预测rating
// 给定n场比赛的表现分,经过第k场比赛后,rating将变为 preSum[k]/k
// 但是当rating超过阈值threshold时就不再涨了
// !给定q个操作,每次操作i,x,表示将第i场比赛的表现分改为x
// 输出每次操作完后,经过这n场比赛后的rating
// 操作是持久化的
// n<=5e5

// 1. 平均数的技巧:平移(减去位移)
// 考虑不修改的情况,找preSum[i]/i>threshold的i
// !即 ∑(perf[i]-threshold)>0,即在新数组上找到最小的i使得前缀和大于0
// 2. 考虑修改，线段树维护前缀和最大值+二分查找 寻找第一个分界点
package main
...
func ratingEstimator(perf []int, ops [][2]int, threshold int) []float64 {
	n := len(perf)
	sums := make([]int, len(perf))
	for i := range sums {
		sums[i] = perf[i] - threshold
	}
	for i := 1; i < len(sums); i++ {
		sums[i] += sums[i-1]
	}

	res := make([]float64, 0, len(ops))
	seg := NewLazySegTree(sums)
	for _, op := range ops {
		index, value := op[0], op[1]
		diff := value - perf[index]
		perf[index] = value // !mutate perf
		seg.Update(index, n, diff)
		right := seg.MaxRight(0, func(data E) bool { return data < 0 })
		if right == n {
			right = n - 1
		}
		res = append(res, float64(threshold)+float64(seg.Get(right))/float64(right+1))
	}
	return res
}
...
func main() {
	...
	var n, threshold, q int
	fmt.Fscan(in, &n, &threshold, &q)
	perf := make([]int, n)
	for i := range perf {
		fmt.Fscan(in, &perf[i])
	}
	...
	res := ratingEstimator(perf, ops, threshold)
	for _, v := range res {
		fmt.Fprintln(out, v)
	}
}
...
func (*LazySegTree) op(left, right E) E     { return max(left, right) }
func (*LazySegTree) mapping(f Id, g E) E    { return g + f }
