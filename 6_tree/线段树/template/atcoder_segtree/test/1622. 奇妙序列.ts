...
class Fancy {
  ...
  append(val: number): void {
    this._seg.update(this._length, this._length + 1, { mul: 1n, add: BigInt(val) })
    this._length++
  }

  addAll(inc: number): void {
    this._seg.update(0, this._length, { mul: 1n, add: BigInt(inc) })
  }

  multAll(m: number): void {
    this._seg.update(0, this._length, { mul: BigInt(m), add: 0n })
  }

  getIndex(idx: number): number {
    if (idx >= this._length) return -1
    return Number(this._seg.get(idx).sum % BIGMOD)
  }
}
