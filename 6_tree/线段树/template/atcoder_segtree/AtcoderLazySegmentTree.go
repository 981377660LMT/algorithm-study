...
func NewLazySegTree(
	...
	n := int64(len(v))
	tree.n = n
	tree.log = int64(bits.Len(uint(n - 1)))
	tree.size = int64(1) << tree.log
...
func (tree *LazySegTree) Query(left, right int64) S {
	if left == right {
...
// 更新切片[left:right]的值
//   0<=left<=right<=len(tree.data)
func (tree *LazySegTree) Update(left, right int64, f F) {
	if left == right {
		return
	}
	left += tree.size
...
func (tree *LazySegTree) MinLeft(right int64, predicate func(data S) bool) int64 {
	if right == 0 {
		return 0
	}
...
func (tree *LazySegTree) MaxRight(left int64, predicate func(data S) bool) int64 {
	if left == tree.n {
		return tree.n
	}
