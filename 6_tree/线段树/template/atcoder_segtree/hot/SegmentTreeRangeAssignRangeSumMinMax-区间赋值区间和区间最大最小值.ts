 ...
 * 区间赋值, 区间和区间最大最小值.
 */
class SegmentTreeRangeAssignRangeSumMinMax {
  ...
  private _propagate(index: number, lazy: number): void {
    ...
    if (index < this._m && lazy !== INF) this._lazy[index] = lazy
  }
}
...
if (require.main === module) {
  ...
  function checkWithBruteForce(): void {
    ...
    const N = 5e4
    const real = new SegmentTreeRangeAssignRangeSumMinMax(Array(N).fill(0))
  ...
  function timeit(): void {
    ...
    const seg = new SegmentTreeRangeAssignRangeSumMinMax(arr)
    console.time('SegmentTreeRangeAssignRangeSumMinMax')
    for (let i = 0; i < n; i++) {
      seg.query(i, n)
      seg.update(i, n, 1)
      seg.set(i, 1)
