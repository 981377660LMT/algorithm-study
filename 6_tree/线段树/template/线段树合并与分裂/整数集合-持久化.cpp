...
int diff( int x, int y, int l = 1, int r = len )        /* 返回集合 x 与集合 y 的差集（x - y），不会改变,→ x 和 y */
{
	int i = ++sz; T[i] = T[x];                      /* 时间复杂度 size(y)log(len) */
	if ( l == r )
	{
...
int intersect( int x, int y, int l = 1, int r = len )   /* 返回集合 x 与集合 y 的交集（不会改变 x,→ 和 y） */
{
...
int symmetric( int x, int y, int l = 1, int r = len )   /* 返回集合 x 与集合 y 的对称差分（不会改,→ 变 x 和 y） */
{
	if ( !x )
