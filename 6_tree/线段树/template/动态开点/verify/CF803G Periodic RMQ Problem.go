...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if op == 1 {
			...
			start--
			operations[i] = [4]int{op, start, end, x}
		} else {
			var start, end int
			fmt.Fscan(in, &start, &end)
			start--
			operations[i] = [4]int{op, start, end, 0}
		}
	}

	res := PeriodicRMQProblem(nums, k, operations)
	for _, v := range res {
		fmt.Fprintln(out, v)
	}
}
...
// 给定一个以nums为循环节，重复k次的序列.
// 1 start end x: 区间赋值
// 2 start end: 区间最小值
// https://www.luogu.com.cn/problem/solution/CF803G
// https://codeforces.com/contest/803/submission/184681591
func PeriodicRMQProblem(nums []int, k int, operations [][4]int) []int {
	n = len(nums)
	nums2 := make([]int, n*2)
	for i := 0; i < n; i++ {
		nums2[i] = nums[i]
	...
	st = _NewSt(nums2, min)
	seg := NewDynamicSegTreeLazy(0, n*k+10, false)
	root := seg.NewRoot()
	...
	for _, op := range operations {
		if op[0] == 1 {
			...
			root = seg.UpdateRange(root, start, end, x)
		} else {
			start, end := op[1], op[2]
			res = append(res, seg.Query(root, start, end))
		}
	}
	return res
}
...
func e1() E { return INF }

// 区间[start,end)的初始值.
func e2(start, end int) E {
	if end-start >= n {
		return st.Query(0, n)
	}
...
func _NewSt(nums []S, op func(S, S) S) *_St {
	res := &_St{}
	n := len(nums)
	b := bits.Len(uint(n))
	st := make([][]S, b)
