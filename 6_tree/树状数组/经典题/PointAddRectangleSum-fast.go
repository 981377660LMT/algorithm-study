...
func main() {
	...
	var n, q int
	fmt.Fscan(in, &n, &q)
	dpars := NewDynamicPointAddRectangleSum(n + q)
	for i := 0; i < n; i++ {
		var x, y, w int
		fmt.Fscan(in, &x, &y, &w)
...
// 根据总点数初始化容量.
func NewDynamicPointAddRectangleSum(n int) *DynamicPointAddRectangleSum {
...
// 返回所有查询结果.
func (dpars *DynamicPointAddRectangleSum) CalculateQueries() []int {
	q := len(dpars.queries)
	rev := make([]int, q)
	sz := 0
	for i := 0; i < q; i++ {
