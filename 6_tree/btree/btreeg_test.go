// https://github.com/tidwall/btree/blob/master/btreeg_test.go#L1C1-L1481C2
// 这段代码构建了一个非常全面的测试套件，用于验证泛型B树（BTreeG）在各种操作下的正确性和稳定性。主要涵盖了以下方面：
//
// 基本操作测试：插入、获取、删除等基本操作的正确性。
// 遍历测试：正向和反向遍历、基于索引的遍历、使用提示的遍历等。
// 边界条件测试：空树操作、单元素操作、重复插入删除等。
// 性能和稳定性测试：大规模随机操作、高负载下的删除操作、并发拷贝等。
// 完整性验证：通过Sane方法确保B树的内部结构和数据一致性。
// 通过这些测试，可以有效地发现和修复B树实现中的潜在问题，确保其在实际应用中的可靠性和性能。
//
// btree-benchmark
