...
func main() {
	array := []uint64{1, 5, 10, 100, 121, 122}
	size := len(array)
	max := array[size-1]
	obj := NewEliasFano(max, uint64(size))
	obj.Compress(array)

	for i := uint64(0); i < obj.Size(); i++ {
		val, _ := obj.Move(i)
		fmt.Println(val)
	}
...
// 将 Elias-Fano 的“高位区”与“低位区”放在同一个 BitSet 里，通过 lowerBitsOffset 划分.
