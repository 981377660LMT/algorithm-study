 ...
 * - count: 离散化(去重)后的元素个数.
 */
function discretizeSparse(nums: number[], offset = 0): [rank: (num: number) => number, count: number] {
  const allNums = [...new Set(nums)].sort((a, b) => a - b)
  // bisect_left
  const rank = (num: number): number => {
    let left = 0
 ...
 * - count: 离散化(去重)后的元素个数.
 */
function discretizeCompressed(nums: number[], offset = 0): [rank: (num: number) => number, count: number] {
  const allNums = [...new Set(nums)].sort((a, b) => a - b)
  const mp = new Map<number, number>()
  for (let index = 0; index < allNums.length; index++) mp.set(allNums[index], index + offset)
  const rank = (num: number) => mp.get(num)!
  return [rank, allNums.length]
}
