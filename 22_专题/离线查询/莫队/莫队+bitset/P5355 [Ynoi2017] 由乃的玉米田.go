...
// 给一个数组，每次询问给定op,start, end, x，(1->差,2->和,3->积,4->商)
// 问区间[start,end)内是否能找出两个数，它们的差/和/积为x。
// 询问次数、值域均不超过1e5
//
// 差和积见 22_专题/离线查询/莫队/莫队+bitset/P3674 小清新人渣的本愿.go
// !判断商k是否存在：根号分治.
// 如果 k>=threshold，直接暴力枚举较小的那个数即可.
// 如果 k<threshold，需要单独拿出来处理.
// 考虑枚举div,O(n)扫一遍回答所有div为某一定值时的答案。具体而言.
// !1.last数组记录每个数上一次出现的位置.
// !2.rightMostLeft[i]表示以位置 i为右端点的所有合法区间中左端点最右的那个区间的左端点位置
// !每访问一个A[i]，就更新last[i]，然后看div*A[i]与A[i]/div这两个数的位置即可.
// !由于遍历常数较小，所以threshold可以取小些.
// 记得特判0的情况(虽然这里没有0).
// 时间复杂度O(n*sqrt(n)+n^2/w).
func 由乃的玉米田(nums []int, queries [][4]int) []bool {
	...
	divThreshold := int(math.Sqrt(float64(max_))/16) + 1
	set := _NewBitset(max_)
	rSet := _NewBitset(max_)
	counter := make([]int, max_+1)
	divTodo := make([][]int, divThreshold+1) // 待统一处理的商为i的查询编号
	...
	mo.Run(
		...
		func(qid int) {
			...
			if op == 1 {
				...
				if k >= divThreshold {
					for a := 1; a*k <= max_; a++ {
						if counter[a] > 0 && counter[a*k] > 0 {
							res[qid] = true
							break
						}
					}
				} else {
					divTodo[k] = append(divTodo[k], qid)
				}
			}
		},
	)

	// !处理商为k的查询, 这里的商k<threshold
	last := make([]int, max_+1)     // last[i]表示i上一次出现的位置
	rightMostLeft := make([]int, n) // rightMostLeft[i]表示以位置 i为右端点的所有合法区间中左端点最右的那个区间的左端点位置
	for div := 1; div < divThreshold; div++ {
		for i := range last {
			last[i] = -1
		}
		for i := range rightMostLeft {
			rightMostLeft[i] = -1
		}
		...
		for i, v := range nums {
			last[v] = i
			if i > 0 {
				rightMostLeft[i] = rightMostLeft[i-1]
			}
			if v%div == 0 {
				rightMostLeft[i] = max(rightMostLeft[i], last[v/div])
			}
			if v*div <= max_ {
				rightMostLeft[i] = max(rightMostLeft[i], last[v*div])
			}
		}
		for _, qid := range divTodo[div] {
			left, right := queries[qid][1], queries[qid][2]-1
			res[qid] = rightMostLeft[right] >= left
		}
	}
