...
func numOfUnplacedFruits(fruits []int, baskets []int) int {
	INF := int(1e18)
	e, op := func() int { return -INF }, func(a, b int) int { return max(a, b) }
	n := int32(len(fruits))
	seg := NewSegmentTreeSqrtDecomposition(e, op, n, func(i int32) int { return baskets[i] }, -1)

	res := n
	for _, v := range fruits {
		right := seg.MaxRight(0, func(_ int32, sum int) bool { return sum < v })
		if right == n {
			continue
		}
		seg.Set(right, -INF)
...
func testTime() {
	...
	fmt.Println("Time1", time.Since(time1)) // Time1 428.791833ms
}
