...
func smallestMissingValueSubtree(parents []int, nums []int) []int {
	...
	findMex := func(sl *SortedList32) int32 {
		left, right := int32(0), sl.Len()-1
		for left <= right {
			mid := (left + right) >> 1
	...
	var dfs func(int32, int32) *SortedList32
	dfs = func(cur, parent int32) *SortedList32 {
		curTree := NewSortedList32(func(a, b int32) bool { return a < b })
		for _, next := range adjList[cur] {
			...
			small.ForEach(func(v int32, _ int32) bool {
				big.Add(v)
				return false
			}, false)
			curTree = big
		...
		curTree.Add(int32(nums[cur]))
		res[cur] = int(findMex(curTree))
		return curTree
	}
