为了补全RadixTree的各个方法，我们设计了一个多级分块结构，其中每个块的大小是基数（blockSize）的幂次。这种结构允许高效地进行区间查询和更新操作。以下是完整的实现：
...
1. **Build方法**：构建多级分块结构。每个层级的分块大小是前一层级的`blockSize`倍。层级0由原数据分块聚合而成，更高层级由前一层级的分块聚合而成。

2. **QueryRange方法**：使用迭代方式处理查询任务，从最高层开始，尽可能利用大块覆盖查询区间，剩余部分分解到低层处理。

3. **Set/Update方法**：更新元素后，逐层更新相关分块的聚合值。每个层级的块索引通过当前块大小计算得到，并重新聚合。

4. **MaxRight/MinLeft方法**：利用分块结构高效地找到满足条件的最大右边界或最小左边界。通过从低到高检查块，尽可能跳过满足条件的块，否则深入更细粒度的块或元素。

该实现通过多级分块结构平衡了查询和更新的效率，适用于需要频繁区间查询和单点更新的场景。
