...
func (sl *SortedListPlus[T]) Erase(x T) {
	...
	sl.blocks[bi] = Replace(sl.blocks[bi], idx, idx+1)
...
func (sl *SortedListPlus[T]) _rangeBitModify(b1, b2 int) {
	...
	for i := b1; i <= b2; i++ {
		if len(sl.blocks[i]) > 0 {
			...
			mx = sl.blocks[i][len(sl.blocks[i])-1]
			sl.blkMax[i] = mx
		} else {
			sl.blkMax[i] = mx
		}
...
func (sl *SortedListPlus[T]) _shrinkBlock(bi int) {
	...
	sl.blocks = Replace(sl.blocks, bi, bi+1)
	sl.blkMax = Replace(sl.blkMax, bi, bi+1)
	sl.bitCnt = Replace(sl.bitCnt, bi, bi+1)
