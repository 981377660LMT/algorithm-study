...
// XorSegTree 异或线段树
// 支持半群的单点修改和区间查询,查询时下标可以异或上 indexXor.
...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if op == 1 {
			...
			var left, right, indexXor int // 0<=left<=right<n
			fmt.Fscan(in, &left, &right, &indexXor)
			right++
			res := seg.Query(left, right, indexXor)
			fmt.Fprintln(out, res.fSum)
		}
	}
}
...
type S = struct{ len, fSum int }

func (*XorSegTree) e() S { return S{} }
func (*XorSegTree) op(a, b S) S { // !f(a+b)=f(a)*11^len(b)+f(b)*2^len(a)
	return S{a.len + b.len, (a.fSum*POW11[b.len] + b.fSum*POW2[a.len]) % MOD}
}
