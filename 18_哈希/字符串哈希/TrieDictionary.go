...
//
// TrieHash（字典树哈希）和字符串哈希是两种不同的字符串处理技术，各有其优缺点。
// TrieHash（字典树哈希）：
// 优点：无哈希冲突，查询效率高，适合处理大量静态数据的查询，如词典、IP路由等。
// 缺点：占用空间大，无法快速求区间哈希值，只能逐位对比，对内存要求较高。
// 字符串哈希：
// 优点：空间复杂度低，可以快速求区间哈希值，适合处理动态变化的数据。
// 缺点：可能存在哈希冲突，即不同的字符串可能得到相同的哈希值，需要额外的方法解决冲突问题。
// 总的来说，选择使用哪种技术取决于具体的应用场景和需求。
//
// api:
//  Add(seq) -> (id, pos)
//  Has(pos) -> bool
