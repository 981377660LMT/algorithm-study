...
func demo() {
	...
	hs := NewDynamicHashStringXReversible(len(s), func(i int) uint { return uint(s[i]) }, base)
...
func abc331_f() {
	...
	hasher1 := NewDynamicHashStringXReversible(len(s), func(i int) uint { return uint(s[i]) }, base)
...
func sumScores(s string) int64 {
	...
	hasher := NewDynamicHashStringXReversible(n, func(i int) uint { return uint(s[i]) }, base)
...
type DynamicHashStringXReversible struct {
	n    int
...
func NewDynamicHashStringXReversible(n int, f func(i int) uint, base *DynamicHashStringBase) *DynamicHashStringXReversible {
	res := &DynamicHashStringXReversible{n: n, base: base}
...
func (hs *DynamicHashStringXReversible) Insert(index int, c uint) {
	hs.root = hs.rbst.Insert(hs.root, uint32(index), fromElement(c))
	hs.n++
}

func (hs *DynamicHashStringXReversible) Pop(index int) uint {
	hs.n--
...
func (hs *DynamicHashStringXReversible) Reverse(start, end int) {
	if start < 0 {
		start = 0
	}
	if end > hs.n {
	...
	hs.root = hs.rbst.Reverse(hs.root, uint32(start), uint32(end))
}

func (hs *DynamicHashStringXReversible) ReverseAll() {
	hs.root = hs.rbst.ReverseAll(hs.root)
}

func (hs *DynamicHashStringXReversible) Get(start, end int) uint {
	if start < 0 {
		start = 0
	}
	if end > hs.n {
...
func (hs *DynamicHashStringXReversible) GetAll() uint { return hs.rbst.QueryAll(hs.root).hash }

func (hs *DynamicHashStringXReversible) Set(index int, c uint) {
...
func (hs *DynamicHashStringXReversible) Size() int { return hs.n }

func (hs *DynamicHashStringXReversible) RotateRight(start, end, k int) {
	if start < 0 {
		start = 0
	}
	if end > hs.n {
...
func (hs *DynamicHashStringXReversible) RotateLeft(start, end, k int) {
	if start < 0 {
		start = 0
	}
	if end > hs.n {
...
func (hs *DynamicHashStringXReversible) RotateRightAll(k int) {
	hs.root = hs.rbst.RotateRightAll(hs.root, uint32(k))
}

func (hs *DynamicHashStringXReversible) RotateLeftAll(k int) {
...
func (rbst *RBSTMonoidNoncommutative) NewNode(v E) *node {
	return &node{v: v, sum: v, revSum: v, size: 1}
}
...
func (rbst *RBSTMonoidNoncommutative) Get(root *node, k uint32) E { return rbst._getRec(root, k, 0) }
...
func (rbst *RBSTMonoidNoncommutative) Reverse(root *node, start, end uint32) *node {
	if start < 0 {
		start = 0
	}
	if n := rbst.Size(root); end > n {
	...
	mid.sum, mid.revSum = mid.revSum, mid.sum
	return rbst._merge3(left, mid, right)
}

func (rbst *RBSTMonoidNoncommutative) ReverseAll(root *node) *node {
	if root == nil {
		return nil
	}
...
func (rbst *RBSTMonoidNoncommutative) _copyNode(n *node) *node {
	...
	return &node{l: n.l, r: n.r, v: n.v, sum: n.sum, revSum: n.revSum, size: n.size, rev: n.rev}
}
...
func (rbst *RBSTMonoidNoncommutative) _propagate(c *node) {
	if c.rev == 1 {
		if c.l != nil {
...
func (rbst *RBSTMonoidNoncommutative) _update(c *node) {
	...
	if left := c.l; left != nil {
		...
		c.revSum = rbst.op(c.revSum, left.revSum)
	}
	if right := c.r; right != nil {
		c.size += right.size
		c.sum = rbst.op(c.sum, right.sum)
		c.revSum = rbst.op(right.revSum, c.revSum)
	}
}
...
func (rbst *RBSTMonoidNoncommutative) _getRec(root *node, k uint32, rev uint8) E {
...
func (rbst *RBSTMonoidNoncommutative) _queryRec(root *node, start, end uint32, rev uint8) E {
...
func test() {
	...
	for i := 0; i < 500; i++ {
		...
		hasher := NewDynamicHashStringXReversible(len(nums), func(i int) uint { return uint(nums[i]) }, base)
		for j := 0; j < 500; j++ {
			// Set
...
func testTime() {
	...
	fmt.Println(time.Since(time1)) // 2.118076791s
}
